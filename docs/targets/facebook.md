...
- saved self-healing extraction scripts live in local `scripts.json`; the post-engagement fast path must extract comments or reactions, otherwise it repairs or falls through to standard extraction; when generation or repair runs, capture a full-page screenshot plus the sanitized HTML snapshot under `artifacts.selfHealing[]`; use `--regenerate-script` when a stale saved script should be skipped for one run
